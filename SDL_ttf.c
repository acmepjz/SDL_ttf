...
/* This function finds the font and index for the specified character,
   using font fallback.
   - If the function succeeds, returns the index of the character,
     and the value `retfont` is updated accordingly.
   - If the function fails, returns 0,
     in this case the value `retfont` is unmodified.
*/
static FT_UInt Get_Char_Index_Using_Font_Fallback(TTF_Font *font, Uint32 ch, TTF_Font **retfont)
{
    int i;
    FT_UInt index;
    ...
    for (i = 0; i < font->num_of_fallbacks; i++) {
        if (font->fallbacks[i] && font->fallbacks[i]->face) {
            index = Get_Char_Index_Using_Font_Fallback(font->fallbacks[i], ch, retfont);
            if (index > 0) {
                return index;
            }
        }
...
static FT_Error Find_Glyph(TTF_Font *font, Uint32 ch, int want) {
    ...
    retfont = font;
    idx = Get_Char_Index_Using_Font_Fallback(font, ch, &retfont);

    retval = Find_GlyphByIndex(retfont, idx, want);

    /* FIXME: Dirty code for font fallback,
    which possibly makes font->current points outside of font! */
    font->current = retfont->current;
...
static int TTF_SizeUTF8_Internal(TTF_Font *font, const char *text, int *w, int *h, int *xstart, int *ystart)
{
    ...
    while (textlen > 0) {
        ...
        /* handle kerning */
        if (font->use_kerning && prev_index && glyph->index && prev_face == glyph->face) {
            FT_Vector delta;
